...
-------------------------------
-- Applies clip planes ( { pos, normal, seal } ) to a construct result, in place. Physics convex
-- clipping is deferred until the entity has a real physics mesh to clip ( see
-- clipMeshByPlanes / PrimitiveRebuildPhysics in entities/base.lua ) -- the planes are just
-- stashed here so both the physics and visual passes cut along the exact same planes.
local function applyClips( result, clips, physics )
    if physics and istable( result.convexes ) then
        result.clipPlanes = clips
    end
    ...
    if CLIENT and istable( result.verts ) and istable( result.index ) then
        for _, clip in ipairs( clips ) do
            ...
            local above = result:Bisect( { pos = clip.pos, normal = clip.normal }, clip.seal, false )
...
--[[
    @FUNCTION: addon.construct.clipMeshByPlanes

    @DESCRIPTION:
        Clips a triangle soup ( verts in groups of 3, as returned per-convex by
        PhysObj:GetMeshConvexes ) against a sequence of clip planes, keeping the normal side of
        each. Reuses simpleton:Bisect -- the same bisection the render mesh is cut with -- so a
        primitive's physics and visuals always agree on exactly where a cut lands.

    @PARAMETERS:
        [table] verts -- flat array of Vectors, 3 per triangle
        [table] clips -- array of { pos, normal, seal }

    @RETURN:
        [table] the clipped verts, or nil if a clip plane erased the mesh entirely
--]]
function addon.construct.clipMeshByPlanes( verts, clips )
    local mesh = simpleton.New()
    mesh.verts = verts
    for i = 1, #verts do mesh.index[i] = i end

    for i = 1, #clips do
        local clip = clips[i]
        local planePos, planeNormal = clip.pos, clip.normal

        -- Cheap side test first: Bisect can't tell "entirely above" ( nothing to do ) apart
        -- from "entirely below" ( the whole thing got cut away ), since both return false.
        local anyAbove, anyBelow = false, false
        for j = 1, #mesh.verts do
            if vec_dot( planeNormal, mesh.verts[j] - planePos ) >= -1e-6 then anyAbove = true else anyBelow = true end
            if anyAbove and anyBelow then break end
        end

        if not anyAbove then return nil end -- plane erased the mesh entirely

        if anyBelow then
            local above = mesh:Bisect( { pos = planePos, normal = planeNormal }, clip.seal, false )
            if above then mesh = above end
        end
    end
...
-- DOME_HOLLOW
-- Pushes interleaved outer+inner hemisphere verts into model over numseg2+1 latitude bands.
-- Rings walk up from the equator, each covering 1/maxseg2 of the 90 degree quarter arc, so
-- numseg2 < maxseg2 leaves the cap open. Longitudes work the same way against maxseg/numseg.
-- Each position v in ring r stores [outer, inner] consecutively; twoRing = 2*(numseg+1) gives the same place on the next ring.
